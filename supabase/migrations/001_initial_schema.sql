...
BEGIN
  ...
  WITH ended_sessions AS (
    SELECT s.id, s.group_filter, s.round_id
    FROM public.sessions s
    WHERE s.status = 'ended' AND s.course_name = p_course_name
  ),
  student_records AS (
    SELECT 
      ar.student_id,
    ...
    JOIN ended_sessions es ON es.id = ar.session_id
  ),
  session_counts AS (
    SELECT 
      st.id AS student_id,
      COUNT(DISTINCT es.id)::BIGINT AS total_sessions
    FROM public.students st
    CROSS JOIN ended_sessions es
    WHERE st.status = 'active'
      AND (es.group_filter IS NULL OR es.group_filter = st.group_label)
    GROUP BY st.id
  ),
  student_stats AS (
    SELECT 
      ...
      COUNT(DISTINCT CASE WHEN sr.method = 'manual' THEN sr.session_id END)::BIGINT AS manual_count,
      COUNT(DISTINCT CASE WHEN sr.method IN ('override_code', 'instructor_approved') THEN sr.session_id END)::BIGINT AS override_count
    FROM public.students st
    LEFT JOIN student_records sr ON sr.student_id = st.id
    WHERE st.status = 'active'
    GROUP BY st.id, st.roll_number, st.name, st.group_label
  )
  SELECT 
    ss.student_id,
    ss.roll_number,
    ...
    ss.manual_count,
    ss.override_count,
    COALESCE(sc.total_sessions, 0)::BIGINT AS total_sessions,
    CASE 
      WHEN COALESCE(sc.total_sessions, 0) = 0 THEN 0.0
      ELSE ROUND((ss.present_count::NUMERIC / sc.total_sessions::NUMERIC) * 100, 1)
