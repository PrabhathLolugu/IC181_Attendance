    ...
    <div className="min-h-screen bg-slate-50 dark:bg-[#0d1117] flex">
        ...
        <div className="relative z-10 flex flex-col h-full p-12">
          <div className="flex items-center gap-3 mb-auto">
            ...
            <div>
              ...
              <p className="text-blue-200 text-xs">Smart QR + GPS Attendance</p>
            </div>
          </div>

          <div className="my-auto">
            <h1 className="text-4xl font-bold text-white leading-tight tracking-tight">
              Attendance,<br /><span className="text-blue-200">verified.</span>
            </h1>
            <p className="text-blue-100 mt-4 text-base leading-relaxed max-w-xs">
              GPS-verified, QR-secured, real-time attendance for your classroom.
            </p>
            <div className="mt-8 flex flex-col gap-3">
              {[
                'Server-verified GPS + rotating signed QR',
                'Live session dashboard with override controls',
                'Attendance sheet synced automatically',
                'Role-based access for admins and TAs',
              ].map((f) => (
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-6 sm:p-12">
        <div className="w-full max-w-md animate-slide-up">
          ...
          {mode === 'signin' ? (
            <>
              <h2 className="text-2xl font-bold text-slate-900 dark:text-slate-100">Staff Sign In</h2>
              <p className="text-slate-500 dark:text-slate-400 mt-1 text-sm">Admins and TAs sign in here.</p>
              ...
              <form onSubmit={handleSubmit} className="mt-8 flex flex-col gap-4">
                ...
                <div>
                  ...
                  <button type="button" onClick={() => { setMode('reset'); setError(''); }} className="mt-1.5 text-xs text-blue-600 hover:text-blue-700 font-medium">
                    Forgot password?
                  </button>
                </div>
            ...
            <>
              ...
              <p className="text-slate-500 dark:text-slate-400 mt-1 text-sm">We'll email you a reset link.</p>

              {resetSent ? (
                <div className="mt-8 flex flex-col gap-4">
                  <div className="flex items-center gap-2 px-4 py-3 bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-200 dark:border-emerald-500/20 rounded-xl text-emerald-700 dark:text-emerald-400 text-sm">
                    Check your inbox for a password reset link.
                  </div>
                  <button onClick={() => { setMode('signin'); setResetSent(false); }} className="btn-secondary w-full">Back to sign in</button>
                </div>
              ) : (
                <form onSubmit={handleReset} className="mt-8 flex flex-col gap-4">
                  ...
                  {error && <div className="px-4 py-3 bg-red-50 dark:bg-red-500/10 border border-red-200 dark:border-red-500/20 rounded-xl text-red-700 dark:text-red-400 text-sm">{error}</div>}
                  <button type="submit" disabled={loading} className="btn-primary w-full h-11">{loading ? 'Sending…' : 'Send reset link'}</button>
                  <button type="button" onClick={() => setMode('signin')} className="text-xs text-slate-400 hover:text-slate-600">← Back to sign in</button>
                </form>
              )}
            </>
          )}
          ...
          <div className="mt-6 p-4 bg-slate-50 dark:bg-[#21262d] border border-slate-200 dark:border-[#30363d] rounded-xl text-center">
            ...
            <p className="text-xs text-slate-700 dark:text-slate-300 mt-0.5 font-medium">Scan the QR code shown in class — no login needed.</p>
          </div>
        </div>
      </div>
    </div>
